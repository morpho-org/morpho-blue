...
contract MorphoStorageLibTest is BaseTest {
    ...
    function testStorage(uint256 amountSupplied, uint256 amountBorrowed, uint256 timeElapsed, uint256 fee) public {
        ...
        bytes32[] memory slots = new bytes32[](18);
        ...
        slots[7] = MorphoStorageLib.lastUpdateSlot(id);
        slots[8] = MorphoStorageLib.feeSlot(id);
        slots[9] = MorphoStorageLib.isIrmEnabledSlot(address(irm));
        slots[10] = MorphoStorageLib.isLltvEnabledSlot(LLTV);
        slots[11] = MorphoStorageLib.isAuthorizedSlot(authorizer, BORROWER);
        slots[12] = MorphoStorageLib.nonceSlot(authorizer);
        slots[13] = MorphoStorageLib.idToBorrowableTokenSlot(id);
        slots[14] = MorphoStorageLib.idToCollateralTokenSlot(id);
        slots[15] = MorphoStorageLib.idToOracleSlot(id);
        slots[16] = MorphoStorageLib.idToIrmSlot(id);
        slots[17] = MorphoStorageLib.idToLltvSlot(id);
        ...
        assertEq(uint256(values[5] >> 128), morpho.totalSupplyShares(id));
        assertEq(uint256(uint128(uint256(values[5]))), morpho.totalSupplyAssets(id));
        assertEq(uint256(values[6] >> 128), morpho.totalBorrowShares(id));
        assertEq(uint256(uint128(uint256(values[6]))), morpho.totalBorrowAssets(id));
        assertEq(uint256(values[7]), morpho.lastUpdate(id));
        assertEq(uint256(values[8]), morpho.fee(id));
        assertEq(abi.decode(abi.encode(values[9]), (bool)), morpho.isIrmEnabled(address(irm)));
        assertEq(abi.decode(abi.encode(values[10]), (bool)), morpho.isLltvEnabled(LLTV));
        assertEq(abi.decode(abi.encode(values[11]), (bool)), morpho.isAuthorized(authorizer, BORROWER));
        assertEq(uint256(values[12]), morpho.nonce(authorizer));
        ...
        assertEq(abi.decode(abi.encode(values[13]), (address)), expectedBorrowableToken);
        assertEq(abi.decode(abi.encode(values[14]), (address)), expectedCollateralToken);
        assertEq(abi.decode(abi.encode(values[15]), (address)), expectedOracle);
        assertEq(abi.decode(abi.encode(values[16]), (address)), expectedIrm);
        assertEq(uint256(values[17]), expectedLltv);
    }
}
