 ...
 *         Copied from https://raw.githubusercontent.com/usmfum/USM/master/contracts/WadMath.sol
 */
library WadMath {
    uint256 public constant WAD = 1e18;
    uint256 public constant WAD_MINUS_1 = WAD - 1;
    ...
    uint256 public constant CEIL_LOG_2_WAD_SCALED = 158961593653514369813532673448321674076; // log2(1e18) * 2**121
    uint256 public constant FLOOR_LOG_2_E_SCALED_OVER_WAD = 3835341275459348169; // log2(e) * 2**121 / 1e18
    uint256 public constant CEIL_LOG_2_E_SCALED_OVER_WAD = 3835341275459348170; // log2(e) * 2**121 / 1e18

    function wadMul(uint256 x, uint256 y, bool roundUp) internal pure returns (uint256 z) {
    ...
    function wadMulDown(uint256 x, uint256 y) internal pure returns (uint256 z) {
        ...
        unchecked {
            z /= WAD;
        } // 369 (3.69) * 271 (2.71) -> 99999 (9.9999) -> 999 (9.99).
    }
    ...
    function wadSquaredDown(uint256 x) internal pure returns (uint256 z) {
        z = x * x;
        unchecked {
            z /= WAD;
        }
    ...
    function wadSquaredUp(uint256 x) internal pure returns (uint256 z) {
        z = (x * x) + WAD_MINUS_1;
        unchecked {
            z /= WAD;
        }
    ...
    function wadDiv(uint256 x, uint256 y, bool roundUp) internal pure returns (uint256 z) {
     ...
     *
     */
    function wadMulDivDown(uint256 w, uint256 x, uint256 y) internal pure returns (uint256 z) {
        z = w * x / y;
    }

    function wadMulDivUp(uint256 w, uint256 x, uint256 y) internal pure returns (uint256 z) {
    ...
    function wadMax(uint256 x, uint256 y) internal pure returns (uint256 z) {
        z = (x > y ? x : y);
    }

    function wadMin(uint256 x, uint256 y) internal pure returns (uint256 z) {
        z = (x < y ? x : y);
    }
     ...
     */
    function wadPowInt(uint256 x, uint256 n) internal pure returns (uint256 z) {
        unchecked {
            z = n % 2 != 0 ? x : WAD;
        }
    ...
    function wadSqrtDown(uint256 y) internal pure returns (uint256 root) {
        root = wadPowDown(y, HALF_WAD);
    }

    function wadSqrtUp(uint256 y) internal pure returns (uint256 root) {
     ...
     *       = e**y * 1e18
     */
    function wadExpDown(uint256 y) internal pure returns (uint256 z) {
        uint256 exponent = FLOOR_LOG_2_WAD_SCALED + FLOOR_LOG_2_E_SCALED_OVER_WAD * y;
     ...
     *
     */
    function wadPowDown(uint256 x, uint256 y) internal pure returns (uint256 z) {
        require(x <= type(uint128).max, "x overflow");
        require(y <= uint256(type(int256).max), "y overflow");
        // The logic here is: Z = pow_2(FLOOR_LOG_2_WAD_SCALED + (log_2(X) - CEIL_LOG_2_WAD_SCALED) * Y / WAD)
        int256 exponent = int256(uint256(log_2(uint128(x))));
        unchecked {
            exponent -= int256(CEIL_LOG_2_WAD_SCALED);
        } // No chance of overflow here, both operands too small
        exponent *= int256(y);
        unchecked {
            exponent = exponent / int256(WAD) + int256(FLOOR_LOG_2_WAD_SCALED);
        } // Can't overflow (would have in prev line)
        require(exponent >= 0, "exponent underflow");
        require(uint256(exponent) <= type(uint128).max, "exponent overflow");
        z = pow_2(uint128(uint256(exponent))); // Apparently Solidity won't let us do this cast in one shot.  Weird eh?
    }

    function wadPowUp(uint256 x, uint256 y) internal pure returns (uint256 z) {
     ...
     */
    function log_2(uint128 x) internal pure returns (uint128 z) {
        unchecked {
            require(x != 0, "x = 0");
            ...
            b = b * b >> 127;
            if (b >= 0x100000000000000000000000000000000 /*b >>= 1;*/ ) l |= 0x10000000000000000;
