    ...
    IMorphoLiquidateCallback
{
    ...
    function netWorth(address user) internal view returns (uint256) {
        ...
        uint256 collateralAssetValue = collateralToken.balanceOf(user).wMulDown(collateralPrice);
    ...
    function testLiquidate(uint256 assetsLent) public {
        ...
        uint256 expectedRepaid = toSeize.wMulUp(collateralPrice).wDivUp(liquidationIncentiveFactor);
        uint256 expectedNetWorthAfter = liquidatorNetWorthBefore + toSeize.wMulDown(collateralPrice) - expectedRepaid;
