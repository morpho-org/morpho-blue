  ...
  it("should simulate gas cost [main]", async () => {
    for (let i = 0; i < signers.length; ++i) {
      ...
      if (random() < 2 / 3) {
        ...
        await blue.connect(user).withdraw(market, amount.mul(totalSupplyShares.add(BigNumber.WAD)).div(totalSupply.add(1)).div(2), user.address);
      } else {
        const totalSupply = await blue.totalSupply(id);
        ...
        if (amount > BigNumber.from(0)) {
          ...
          await blue.connect(user).repay(market, amount.mul(totalBorrowShares.add(BigNumber.WAD)).div(totalBorrow.add(1)).div(4), user.address);
